...
class DownloadRoutine {
  ...
  private async downloadCover(url: string, filePath: string): Promise<string> {
    try {
  ...
  public async run(tracks: SpotifyTrack[]): Promise<void> {
    for (const track of tracks) {
      ...
      const isSongDownloaded = await this.downloadFile(videoId, filePath, fileName);
      if (!isSongDownloaded) {
        console.log(`Não foi possível efetuar o download de ${query}, indo para o próximo...`)
        continue;
      }

      const isCoverDownloaded = await this.downloadCover(track.coverUrl, thumbPath);
